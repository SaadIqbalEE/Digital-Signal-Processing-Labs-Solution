/* dsplab.c - common definitions and functions
...
void initialize()                       
{
   ...
   MCBSP_FSETS(SPCR1, RINTM, FRM);                       // reconfigure for 32-bit word transfers
   MCBSP_FSETS(SPCR1, XINTM, FRM);
   MCBSP_FSETS(RCR1, RWDLEN1, 32BIT);
   MCBSP_FSETS(XCR1, XWDLEN1, 32BIT);

   IRQ_globalDisable();
   IRQ_map(IRQ_EVT_RINT1, 11);			// IRQ_map(IRQ_EVT_XINT1, 11); works too, why?
   IRQ_reset(IRQ_EVT_RINT1);    	        // IRQ_reset(IRQ_EVT_XINT1);   works too, why?		  
   IRQ_globalEnable();       			
   IRQ_nmiEnable();          			
   IRQ_enable(IRQ_EVT_RINT1);			// IRQ_enable(IRQ_EVT_XINT1);  works too, why?

   MCBSP_write(DSK6713_AIC23_DATAHANDLE, 0);                 //write an output sample
